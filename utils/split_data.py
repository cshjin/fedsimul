...
def create_jsons_for(user_files, which_set, max_users, include_hierarchy):
    ...
    for (i, t) in enumerate(user_files):
        ...
        if (user_count == max_users) or (i == len(user_files) - 1):
            ...
            ouf_dir = os.path.join(data_path, which_set, file_name)
...
data_path = os.path.join(tmpdir, 'data', args.name)
subdir = os.path.join(data_path, 'rem_user_data')
files = []
if os.path.exists(subdir):
    files = os.listdir(subdir)
if len(files) == 0:
    subdir = os.path.join(data_path, 'sampled_data')
    if os.path.exists(subdir):
        files = os.listdir(subdir)
if len(files) == 0:
    subdir = os.path.join(data_path, 'all_data')
    files = os.listdir(subdir)
...
if (args.user):
    ...
    for f in files:
        ...
        ouf_dir_train = os.path.join(data_path, 'train', file_name_train)
        ouf_dir_test = os.path.join(data_path, 'test', file_name_test)
